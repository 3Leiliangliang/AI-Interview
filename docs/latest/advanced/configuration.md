...
AI-interview 采用了现代化的配置管理系统，基于 Pydantic BaseModel 和 TOML 格式，提供了类型安全、智能提示和选择性持久化等特性。这套系统既满足了开发者对灵活配置的需求，又保证了运行时的稳定性。

## 设计理念

传统的配置文件往往存在以下问题：格式不统一、类型安全缺失、难以追踪哪些配置是用户修改过的。AI-interview 的配置系统针对这些问题给出了解决方案：
...
1. **代码默认值**：定义在 `src/config/app.py` 和 `src/config/static/models.py` 中，提供所有配置项的初始值
